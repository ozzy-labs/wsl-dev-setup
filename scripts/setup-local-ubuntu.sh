...
# 5. コンテナツール（Dev Container開発の中核）
# 6. クラウドツール（unzipに依存）
# 7. AIツール（Claude Code/Copilot CLIはcurlに依存、Codex/Gemini CLIはnpmに依存）
# 8. 開発補助ツール
...
install_mise_and_languages() {
  # Node または Python のいずれかが必要な場合のみ mise を導入
  if [ "$INSTALL_NODE" != "1" ] && [ "$INSTALL_PYTHON" != "1" ]; then
    return
  fi

  echo ""
  echo "⚡ mise（バージョン管理）をインストール中..."
...
# ========================================
# 注意: 依存関係の順序で実行されます
# 1. ビルドツール → 2. 基本CLIツール → 3. Git → 4. mise + 言語環境
# → 5. コンテナツール → 6. クラウドツール → 7. AIツール → 8. 開発補助ツール
